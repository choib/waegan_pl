...
class WaeGAN(LightningModule):
    ...
    def forward(self, x, noise, label):
        return self.generator_dec(self.generator_enc(x), noise, label)

    def compute_gradient_penalty_u(self, real_samples, fake_samples):
    ...
    def compute_gradient_penalty_v(self, real_samples, fake_samples):
        """Calculates the gradient penalty loss for WGAN GP"""
        # Random weight term for interpolation between real and fake samples
        alpha = torch.Tensor(np.random.random((real_samples.size(0), 1, 1, 1))).to(self.device)
        # Get random interpolation between real and fake samples
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        gen_labels = Variable(LongTensor(np.random.randint(0, self.n_classes,real_A.shape[0])),requires_grad=False)
        z = Variable(FloatTensor(np.random.normal(0, 1, (labels.shape[0],self.args.n_classes))))
        ...
        if optimizer_idx == 0 :
            ...
            #e1_ = downstream[-2]
            #e2_ = downstream[-1]
            generated, _, _, _ = self.generator_dec(self.generator_enc(real_A.detach()))#downstream)
            ...
            if self.args.clip_weight:
                ...
                gradient_penalty = self.compute_gradient_penalty_u(real_B.data, generated.data)
            ...
            self.log("discriminator_v loss",dv_loss, sync_dist=True)
            
            tqdm_dict = {'dv_loss': dv_loss.detach()}
    ...
    def test_step(self, batch, batch_idx):
        for img_A, img_B, aug_A, pathA, pathB in zip(batch["A"], batch["B"], batch["aug_A"], batch["pathA"], batch["pathB"]):
            ...
            fake_B, e, e1, e2 = self.generator_dec(self.generator_enc(real_A))
            _, z, z1, z2 = self.generator_dec(self.generator_enc(aug_A))
        ...
        tqdm_dict = {'test_loss': test_loss.detach()}
    ...
    def predict_step(self, batch, batch_idx):
        for img_A, aug_A, pathA in zip(batch["A"], batch["aug_A"], batch["pathA"]):
            ...
            fake_B, e, e1, e2 = self.generator_dec(self.generator_enc(real_A))
            #_, z, z1, z2 = self.generator_dec(self.generator_enc(aug_A))
            nz_f = self.mse_loss(e, fake_B) #+ self.mse_loss(e1, z1) + self.mse_loss(e2, z2)
            nz_f = nz_f.item()
...
class SaveImage(Callback):
    ...
    def on_train_epoch_end(self, trainer: Trainer, pl_module: LightningModule) -> None:
        ...
        pl_module.generator_enc.eval()
        pl_module.generator_dec.eval()
        pl_module.freeze()
        val_loss, validity = sv.sample_images(batches_done, self.test_loader, self.args, pl_module.generator_enc, pl_module.generator_dec, pl_module.mse_loss, Tensor)
        self.log("validation loss",val_loss)
        self.log("validity",validity)
        pl_module.generator_enc.train()
        pl_module.generator_dec.train()
        pl_module.unfreeze()
...
def main(args: Namespace) -> None:
    ...
    trainer = Trainer(gpus=args.gpu,strategy=accel,callbacks=callbacks,\
