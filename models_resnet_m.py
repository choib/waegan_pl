...
class UNetDown(nn.Module):
    def __init__(self, in_size, out_size, normalize=True, dropout=0.0, swish_act=False, relu_act=False, style=False):
...
class Discriminator_CNN(nn.Module):
    ...
    def __init__(self, args, wass_metric=True):
        super(Discriminator_CNN, self).__init__()
        
        self.name = 'discriminator'
        self.channels = 1 # z:1 d1:64 d2:128
        ...
        def discriminator_block(in_filters, out_filters, normalize=self.normalize, relu_act=self.relu_act):
        ...
        self.fc1 = nn.Linear(self.n_z, self.img_width * self.img_height // 4)
        self.fc2 = nn.Linear(self.n_z, self.img_width * self.img_height // 16)
        #self.fc2.weight.data = torch.ones(self.img_width * self.img_height // 16,self.n_z)
        #self.fc2.bias.data = torch.zeros(self.img_width * self.img_height // 16)
    ...
    def forward(self, z):
        z0 = z.view(z.size(0),1,z.size(1),-1)
        validity = self.model(z0)
        return validity

    def compute_gradient_penalty(self, real_data, generated_data):
        ...
        #alpha = torch.rand(b_size, self.n_z)
        ...
        sum_gradients_norm = 0
        for prob_interpolated in self.forward(interpolated):
        ...
        # the square root, so manually calculate norm and add epsilon
            gradients_norm = torch.sqrt(torch.sum(gradients ** 2, dim=1) + 1e-15)
            #gradients_norm = (((gradients + 1e-16).norm(2, dim=1) - 1 ) ** 2).mean()
            #gradients_norm = (((gradients + 1e-16).norm(2, dim=1)) ** 2).mean()
            
            sum_gradients_norm += gradients_norm
           
        # Return gradient penalty
        return ((sum_gradients_norm-1)**2).mean() 
...
class MultiDiscriminator(nn.Module):
    ...
    def compute_out(self, generated_data):
        #loss = sum([torch.mean(gen) for gen in self.forward(generated_data)])
        loss = sum([gen for gen in self.forward(generated_data)])
        return loss    
